...
def find_matching_pairs_vectorized_optimized_parallel(file_path, min_matching_nmers=3, n=3, batch_size=50000,
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        ...
        # Add a start_batch parameter to your function with default value (e.g., 0)
        start_batch = 262  # Define this at the beginning of your function or pass it as a parameter
        
        for batch_idx, id_indices in enumerate(balanced_batches):
            # Skip batches with index less than start_batch
            ...
            batch_jobs.append((batch_idx, job))  # Keep track of batch index with the job

        # Print info about skipped batches
        print(f"Skipped {skipped_batches} batches (batch indices 0-{start_batch-1})")
        print(f"Will process {len(batch_jobs)} remaining batches")
        ...
        # Adjust progress bar to show only remaining batches
        batch_pbar = tqdm(total=len(batch_jobs), desc="Processing batches",
                        mininterval=0.5, maxinterval=3.0, leave=True,
                        bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]',
                        ncols=100, position=0)

        # Process each job and track which batch it belongs to
        for batch_idx, job in batch_jobs:
            ...
            remaining_batches = len(batch_jobs) - completed_batches
            ...
            # Print batch index for tracking
            print(f"Completed batch {batch_idx} ({completed_batches}/{len(batch_jobs)})")
    ...
    print(f"Processed batches: {len(batch_jobs)} (starting from batch {start_batch})")
