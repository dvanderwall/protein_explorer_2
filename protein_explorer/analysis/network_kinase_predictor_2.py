...
def get_similar_sites(site_id: str, 
    ...
    if not site_data:
        ...
        # Try to get site data from database
        site_data = get_phosphosite_data(site_id)
    
    if site_data:
        # Extract from site_data
        site_number = site_data.get('Residue_Number')
        # Try to get site_type from site_data
        if 'Residue' in site_data:
            site_type = site_data['Residue']
        elif 'site_type' in site_data:
    ...
    try:
        # Get structural matches from database
        struct_matches = find_structural_matches(site_id, rmsd_threshold)
        
        for match in struct_matches:
            target_id = match.get('target_uniprot', '') + '_' + match.get('target_site', '').replace('S', '').replace('T', '').replace('Y', '')
...
def get_network_kinase_scores(site_id: str, score_type: str = 'structure',
    ...
    for site in similar_sites:
        site_scores = all_scores_batch.get(site)
